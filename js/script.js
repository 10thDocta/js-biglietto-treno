...
const calcPrice = (kmInput, ageInput) => {
  if (isNaN(kmInput) || isNaN(ageInput)) {
    ...
    price.innerHTML = `${(currentPrice - (currentPrice * 20) / 100).toFixed(1)} €`;
  } else if (ageInput > 65) {
    price.innerHTML = `${(currentPrice - (currentPrice * 40) / 100).toFixed(1)} €`;
  } else {
    price.innerHTML = `${currentPrice.toFixed(1)} €`;
  }
};
